...
def import_cardcodes(filepath = "rapidsmsrw1000/apps/ubuzima/xls/cardcodes.xls", sheetname = "Codes", keyrow = 1, descrow = 2, catrow = 3, valrow = 4):
    ...
    for row_index in range(sheet.nrows):
        if row_index < 1: continue
        key     = sheet.cell(row_index,keyrow-1).value
        desc    = sheet.cell(row_index,descrow-1).value
        cat     = sheet.cell(row_index,catrow-1).value
        value   = sheet.cell(row_index,valrow-1).value
        try:
            category,created = FieldCategory.objects.get_or_create( name = cat)
            ft,created = FieldType.objects.get_or_create( key = key)
            ft.key = key.lower()
            ft.description = desc
            ft.category = category
            ft.has_value = True if value.upper() == "YES" else False
            ft.save()          
        except Exception, e:
            pass
    return True
