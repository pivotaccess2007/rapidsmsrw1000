...
function prepData(conf) {
	...
	wfa_haiti_0_to_5_values = [{"Month":0,"normal":3.4,"mal":2.4,"smal":2},{"Month":6,"normal":8,"mal":5.5,"smal":4.6},{"Month":12,"normal":10.3,"mal":7.8,"smal":6.5},{"Month":18,"normal":11.5,"mal":8.7,"smal":7.3},{"Month":24,"normal":12.5,"mal":9.6,"smal":8.1},{"Month":30,"normal":13.7,"mal":10.5,"smal":9},{"Month":36,"normal":14.8,"mal":11.4,"smal":9.8},{"Month":42,"normal":15.7,"mal":12,"smal":10.3},{"Month":48,"normal":16.7,"mal":12.8,"smal":10.9},{"Month":54,"normal":17.8,"mal":13.5,"smal":11.5},{"Month":60,"normal":18.7,"mal":14.2,"smal":12}];


	wfa_boys_0_to_5_meta = {};
	wfa_boys_0_to_5_meta.lines =  conf.wfa_all_0_to_5_meta.lines.slice();
	wfa_boys_0_to_5_meta.title = "Weight for Age, Boys";
...
function display_growth_chart(growthData, el, chartType, config) {
  ...
  // TODO replace with vanillajs and get rid of underscore
  if (_.find(['wfa_boys_0_to_5', 'wfa_girls_0_to_5'], function(k) { return k === chartType; })){
	  color.domain([0, 1, 2, 3, 4, 5, 6])
			.range([red, yellow, green, green, yellow, red, red]);
	} else {
  // TODO replace with vanillajs and get rid of underscore
  if (_.find(['wfa_boys_2_to_20', 'wfa_girls_2_to_20'], function(k) { return k === chartType; })){
	  color.domain([0, 1, 2, 3, 4, 5])
			.range([yellow, green, green, yellow, red]);
	} else {
		// this is for Weight vs Age (Haitian Minstry of Health)
	  color.domain([0, 1, 2])
  ...
  // Patient's data
  // TODO replace with vanillajs and get rid of underscore
	var patientData = _.map(growthData, function(p) {return [p.age, p.weight];});
