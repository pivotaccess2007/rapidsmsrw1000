...
DimZum.Controller = Class.create({
  ...
  getElementsFromAnchor: function(anchor) {
    return anchor.substring(this.prefix.length + 1).split("index.html").map(function(id) { return $(id) });
  },
...
(function() {
  ...
  Beanstalk.beacon = {
    ...
    log: function(params) {
      ...
      for (key in params) {
        value = params[key];
        _results.push(Beanstalk.beacon.params[key] = value);
      }
      return _results;
    },
    send: function() {
      var image, key, params, value;
  ...
  if (Beanstalk.enablePageviewTracking) {
    ...
    if (document.addEventListener) {
      ...
      contentloadtag.onreadystatechange = function() {
        if (this.readyState === 'complete') return Beanstalk.logPageview();
      };
    }
  }
...
(function() {
  ...
  Beanstalk.Timing = {
    ...
    findStartTime: function() {
      ...
      if (startTime) return Beanstalk.Timing.mark("starttime", startTime);
    },
...
var zoomImagesURI   = 'images/zoom/index.html'; // Location of the zoom and shadow images
...
function sendOffsite(link, urchinTrackerName) {
	if (urchinTrackerName != undefined) {
		...
		masterTracker._trackPageview(link.href.split('index.html')[2]);
		siloTracker._trackPageview(link.href.split('index.html')[2]);
	}
    ...
    if(document.referrer != "") {
      if(document.referrer.match("google") != null) {
        source = "google-" + document.referrer.split("q=")[1].split("&")[0]
          ...
          referring_host = document.referrer.split("//")[1].split("index.html")[0].replace("www.", "")
