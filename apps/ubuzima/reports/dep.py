...
#        muac = m.group(9)   
    ...
    @keyword("\s*dep(.*)")
    def dep(self, message, notice):
        if not getattr(message, 'reporter', None):
            message.respond(_("You need to be registered first, use the REG keyword"))
            return True
    	rez = re.match(r'dep\s+(\d+)', message.text, re.IGNORECASE)
    	if not rez:
    	    message.respond(_('You never reported a departure. Departure are \
reported with the keyword DEP'))
    	    return True
    	ref = Departure(reporter = message.reporter, depid = rez.group(1))
    	ref.save()
