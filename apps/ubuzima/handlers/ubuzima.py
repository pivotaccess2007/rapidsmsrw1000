...
class UbuzimaHandler (KeywordHandler):
    ...
    def handle (self, message):
        ...
        if results:
            try:
    	    	...
    	    	if rpt:
                    err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Unknown Error"), errby = rpt, village=rpt.village, district = rpt.location.district, province = rpt.location.province, nation = rpt.location.nation)
                    err.save()
    	    	else: 
    	    ...
    	    if rpt:
    	    	err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Unknown Keyword"), errby = rpt, village=rpt.village, district = rpt.location.district, province = rpt.location.province, nation = rpt.location.nation)
    	    	err.save()
    	    else: 
    ...
    def sup_or_reg(self, message, keyword, rest):
        ...
        if not m:
    	    ...
    	    err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Incomplete Report"), errby = \
				message.reporter, identity = message.connection.identity, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    	    err.save()
            return True
        ...
        if len(received_nat_id) != 16:
    	    ...
    	    err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Invalid ID"), errby = message.reporter, identity =\
				message.connection.identity, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    	    err.save()
            return True
    ...
    def read_fields(self, reporter, code_string, accept_date=False, weight_is_mothers=False, height_is_mothers=False):
        ...
        for code in codes:
            try:
                ...
                fields.append(Field(type=field_type, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation))
                7
            ...
            # didn't recognize this code?  then it is a scalar value, run some regexes to derive what it is
    	    	...
    	    	m3 = re.search("(\d+\.?\d*)(m|meter|meters)", code, re.IGNORECASE)
                
                # this is a weight
                if m1:
                    field_type = FieldType.objects.get(key="child_weight" if not weight_is_mothers else "mother_weight")
    	    	    if float(m1.group(1)) > 30.0 : field_type = FieldType.objects.get(key="mother_weight")# Didier thinks no child of 24 months above 30kg
                    value = Decimal(m1.group(1))
                    field = Field(type=field_type, value=value, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
                    fields.append(field)
                    
                # this is a length
                elif m2:
                    field_type = FieldType.objects.get(key="muac")
    	    	    if float(m2.group(1)) > 35.00 : field_type = FieldType.objects.get(key="child_height")# Didier thinks no child of 24 months above 1m
                    value = Decimal(m2.group(1))
                    field = Field(type=field_type, value=value, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
                    fields.append(field)
    	    	# this height, either for the mother or the child
    	    	elif m3:
                    field_type = FieldType.objects.get(key="child_height" if not height_is_mothers else "mother_height")
    	    	    if float(m3.group(1)) > 1.00 : field_type = FieldType.objects.get(key="mother_height")# Didier thinks no child of 24 months above 1m
                    value = Decimal(m3.group(1))
                    field = Field(type=field_type, value=value, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
                    fields.append(field)
                ...
                # unknown
                    ...
                    try:
    	    	    	...
    	    	    	if datetime.strptime(date, "%d.%m.%Y").date().year < 1900:	raise Exception(_("Invalid date, Year less than 1900."))
                    except Exception,e:
    ...
    def get_or_create_patient(self, reporter, national_id):
        ...
        try:
            ...
            patient = Patient.objects.create(national_id=national_id,
                                             location=reporter.location, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
	...
	if phone and len(phone.group(1)) == 10:
        ...
        # read our fields
        try:
            (fields, dob) = self.read_fields(reporter,optional_part, False, True)
        except Exception, e:
    	...
    	#calculate our bmi if we have height and weight            
    	try:
            w = report.fields.get(type__key = "mother_weight")
    	    h = report.fields.get(type__key = "mother_height")
    	    bmi = Field(type = FieldType.objects.get(key = 'bmi'), value = w.value/(h.value*h.value), village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    	    report.fields.add(bmi)
    	    report.bmi_anc1 = bmi.value
    	    bmi.save()
    	    report.save()
        except Exception, e:
            pass
    ...
    def birth(self, message, notice):
   	...
   	reporter = message.reporter
        ...
        # read our fields
        try:
            (fields, dob) = self.read_fields(reporter, optional_part, True)
        except Exception, e:
            # there were invalid fields, respond and exit
            message.respond("%s" % e[0])
    	    for er in e[1]:
        ...
        fields.append(Field(type=child_num_type, value=Decimal(received_child_num), village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation))
...
#Didier new keys implementation
    ...
    def je_m_en_fou(self, message, notice):
    	...
    	if not rez:
    	    ...
    	    err.save()
    	    return True
    	ref = Refusal(reporter = message.reporter, refid = rez.group(1), village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    	ref.save()
    	...
    	try:
    		...
    		if mother is None:
    			message.respond(_("The MOTHER_ID is suspected incorrect."))
    	    		err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Invalid ID"), errby = message.reporter, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    			err.save()
    			return True
   		...
   		tr.response=m.group(2).upper() if m.group(2).upper() in [tr.RESPONSE_YES,tr.RESPONSE_NO] else tr.RESPONSE_NR
    		...
    		message.respond(_("The MOTHER_ID is suspected incorrect."))
    	    	err = ErrorNote(errmsg = message.text, type = ErrorType.objects.get(name = "Invalid Information"), errby = message.reporter, village=reporter.village, district = reporter.location.district, province = reporter.location.province, nation = reporter.location.nation)
    		err.save()    		
		return True
    	...
    	try:
   		...
   		tr.response = m.group(2).upper() if m.group(2).upper() == tr.RESPONSE_PO else tr.RESPONSE_NR
    		if tr.response == "PO":
 			...
 			(fields, dob) = self.read_fields(reporter, m.group(2))
    			for field in fields:
    				field.save()
	...
	if not pnc_report and not pnc_dep:
    	...
    	if pnc_report:
	    	...
	    	# read our fields
	try:
	    (fields, dob) = self.read_fields(reporter, optional_part,False, False)
	except Exception, e:
	    # there were invalid fields, respond and exit
	    message.respond("%s" % e[0])
    	    for er in e[1]:
