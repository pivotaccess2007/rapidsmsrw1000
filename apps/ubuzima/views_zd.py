...
from ubuzima.newborn_ind import *
...
def user_hc(req):
    try:
        p = UserLocation.objects.get(user=req.user)
        if p.location.type.name == "Nation" :    hc = Location.objects.filter(nation = p.location, type__name = 'Health Centre')
        elif p.location.type.name == "Province" :    hc = Location.objects.filter(province = p.location, type__name = 'Health Centre')
        elif p.location.type.name == "District" :    hc = Location.objects.filter(district = p.location, type__name = 'Health Centre')
        else : hc = Location.objects.filter(pk = p.location.pk).extra(select = {'selected':'id = %d' % (p.location.pk,)}).order_by('name')
    except UserLocation.DoesNotExist,e:
        return render_to_response(req,"404.html",{'error':e})
...
def user_dis(req):
    try:
        p = UserLocation.objects.get(user=req.user)
        if p.location.type.name == "Nation" :    hc = Location.objects.filter(nation = p.location, type__name = 'District')
        elif p.location.type.name == "Province" :    hc = Location.objects.filter(province = p.location, type__name = 'District')
        else : hc = Location.objects.filter(pk = p.location.district.pk).extra(select = {'selected':'id = %d' % (p.location.district.pk,)}).order_by('name')
    except UserLocation.DoesNotExist,e:
        return render_to_response(req,"404.html",{'error':e})
...
def user_prv(req):
    try:
        p = UserLocation.objects.get(user=req.user)
        if p.location.type.name == "Nation" :    hc = Location.objects.filter(nation = p.location, type__name = 'Province')
        else : hc = Location.objects.filter(pk = p.location.province.pk).extra(select = {'selected':'id = %d' % (p.location.province.pk,)}).order_by('name')
    except UserLocation.DoesNotExist,e:
        return render_to_response(req,"404.html",{'error':e})
...
def anc_report(req):
    ...
    resp['reports'] = paginated(req, new_qryset, prefix="rep")
    #resp['reports'] = paginated(req, qryset, prefix="rep")
